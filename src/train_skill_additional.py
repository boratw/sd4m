...
task_num = 4
training_task = [3]
task_latent = [0.0, 0.0, 0.0, 0.0]

legs = [0, 1, 2, 6]

envs = [[AntGoalLegEnv(i, [1., 1., 1., 1.]) for _ in range(env_num)] for i in legs]
...
learner = Skill2_Learner(state_len, action_len, traj_len, traj_track_len, latent_len, latent_body_len, qpos_len, task_num,
    sampler_diverse=1., sampler_disperse=0., learner_lr=0.001, sampler_lr=0.001, learner_unity=0.01, learner_diverse=0.01, learner_action_magnify=0.1)
follower = [Skill2_Follower(state_len,  action_len, str(i)) for i in range(task_num)]


LOG_DIR = "data/skill2/train12_4/" + CreateLogPrefix()
...
for i in range(task_num):
    log_file.write("Step"+str(i)+"\tMove"+str(i)+"\tReward"+str(i)+"\tsac_score"+str(i)+"\tlearner_score"+str(i))
log_file.write(learner.log_caption())
for i in training_task:
...
sac_score = [0. for _ in range(task_num)]
latent_score = [(100. if task in training_task else -1.) for task in range(task_num)]
...
with sess.as_default():
    ...
    learner_saver.restore(sess, "data/skill2/train12/2021-09-05_00-45-41_log1_learner_3100.ckpt")
    follower_savers[0].restore(sess, "data/skill2/train12/2021-09-05_00-45-41_log1_follower0_3100.ckpt")
    follower_savers[1].restore(sess, "data/skill2/train12/2021-09-05_00-45-41_log1_follower1_3100.ckpt")
    follower_savers[2].restore(sess, "data/skill2/train12/2021-09-05_00-45-41_log1_follower2_3100.ckpt")
    follower_savers[3].restore(sess, "data/skill2/train12/2021-09-05_00-45-41_log1_follower0_3100.ckpt")
    ...
    for episode in range(1, 100001):
        ...
        for task in range(task_num):
            sample_in_latent = (sac_score[task] < latent_score[task] * 0.9)
            if sample_in_latent:
                latent_action = np.tile([[task_latent[task]]], (env_num, 1)) + np.random.normal(0., 0.01, (env_num, 1))
            batch = 0
            while batch < 2 or len(history_learner[task]) < 4 * learner_batch:
                ...
                if sample_in_latent:
                    reward_env = [0. for _ in range(env_num)]
                ...
                for step in range(horizon):
                    ...
                    if sample_in_latent:
                        if batch == 0:
                            ...
                            actions = learner.get_action_batch(np.concatenate([states[:, :2] - first_traj_pos, states_noxy], axis=1),
                                np.concatenate([latent, latent_action], axis=1), True)
                    else:
                        actions = follower[task].get_action_batch(states_noxy, regulized_goal, False)
                        if batch == 0:
                            for i in range(env_num):
                    ...
                    for i in range(env_num):
                        ...
                        if sample_in_latent:
                            reward_env[i] += reward
                        if task in training_task and survive_vector[i]:
                ...
                if sample_in_latent:
                    for i in range(env_num):       
                        if batch == 0:
                            sac_score[task] = sac_score[task] * 0.99 + reward_env[i] * 0.01
                        else:
                            if reward_env[i] > latent_score[task]:
                                change = (reward_env[i] - latent_score[task]) * 0.01 / latent_score[task]
                                task_latent[task] = task_latent[task] * ( 1. - change ) + latent_action[i][0] * change 
                            latent_score[task] = latent_score[task] * 0.99 + reward_env[i] * 0.01
                batch += 1
                ...
                print("Episode " + str(episode) + " Task " + str(task) +  " Step " + str(cur_batch_step )+  " Move " +  str(cur_batch_move )
                    ...
                    + (" Latent_Sample " + str(task_latent[task]) + " " + str(latent_score[task]) if sample_in_latent and batch != 0 else "") )

                if sample_in_latent != 0 or batch != 0 :
                    cur_move[task] += cur_batch_move
                    cur_step[task] += cur_batch_step
                    cur_reward[task] += cur_batch_reward

            if sample_in_latent:
                cur_step[task] /= (batch-1)
                cur_move[task] /= (batch-1)
                cur_reward[task] /= (batch-1)
            else:
                cur_step[task] /= batch
                cur_move[task] /= batch
        ...
        for i in range(task_num):
            log_file.write("\t" + str(cur_step[i]) + "\t" + str(cur_move[i])  + "\t" + str(cur_reward[i]) + "\t"
                + str(sac_score[i]) + "\t" + str(latent_score[i]))
