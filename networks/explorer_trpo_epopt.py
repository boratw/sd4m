...
class Explorer_TRPO:
    def __init__(self, state_len, action_len, goal_len, name="", env_num=1, param_length=1, policy_gamma=0.99, lam=0.98, kl_targ=0.003, hid1_mult=10,
        policy_logvar=-1.0, restore_mean=None, restore_var=None, min_trajectory=2) :


        self.name = "Explorer_TRPO" + name
        with tf.variable_scope(self.name): 
            self.val_func = NNValueFunction(state_len + goal_len + 1 + param_length, hid1_mult)
            self.policy = Policy(state_len + goal_len + 1 + param_length, action_len, kl_targ, hid1_mult, policy_logvar)
    ...
    def get_action(self, input_state, input_goal, input_param, record=True):
        time_step_array = np.array([ [self.time_step] for _ in range(self.env_num)])
        obs = np.concatenate([np.array([input_state]), np.array([input_goal, input_param]), time_step_array], axis=1)
    ...
    def get_action_batch(self, input_state, input_goal, input_param, survive_vector=None, record=True):
        time_step_array = np.array([ [self.time_step] for _ in range(self.env_num)])
        obs = np.concatenate([np.array(input_state), np.array(input_goal), np.array(input_param), time_step_array], axis=1)
        action = self.policy.sample(obs)
        if record:
    ...
    def optimize_batch(self, epsilon=1.0):
        ...
        if epsilon < 1.0:
            advantage_sum = [np.sum(t['advantages']) for t in self.trajectories]
            m = np.percentile(advantage_sum, epsilon * 100)
            self.trajectories = [self.trajectories[i] for i in range(len(advantage_sum)) if advantage_sum[i] < m ]
