...
class Policy(object):
    ...
    def sample_with_dist(self, obs):
        """Draw sample from policy distribution"""
        feed_dict = {self.obs_ph: obs}

        return self.sess.run((self.sampled_act, self.means, self.log_vars), feed_dict=feed_dict)
    ...
    def update(self, observes, actions, advantages):
        ...
        return {'PolicyLoss': loss,
                    'PolicyEntropy': entropy,
                    'KL': kl,
                    'Beta': self.beta,
                    '_lr_multiplier': self.lr_multiplier}

    def update_with_dist(self, observes, actions, advantages, oldmean, oldvar):
        """ Update policy based on observations, actions and advantages
        ...
        feed_dict = {self.obs_ph: observes,
                     ...
                     self.eta_ph: self.eta,
                     self.lr_ph: self.lr * self.lr_multiplier,
                     self.old_log_vars_ph: oldmean,
                     self.old_log_vars_ph: oldvar }
        feed_dict[self.old_means_ph] = old_means_np
        loss, kl, entropy = 0, 0, 0
        for e in range(self.epochs):
            # TODO: need to improve data pipeline - re-feeding data every epoch
